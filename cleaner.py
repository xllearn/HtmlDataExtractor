...
from columns import DATE_COLUMNS, TARGET_COLUMNS
...
def fill_missing_by_field(record: Dict[str, Any], mapping: FieldMapping) -> Dict[str, str]:
    ...
    for column in mapping.output_columns:
        if is_missing(normalized.get(column)):
...
def deduplicate_records(records: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for record in records:
        key = "|".join(clean_text(record.get(column)) for column in DEDUP_COLUMNS)
        if not key.strip("|"):
            key = "|".join(clean_text(record.get(column)) for column in TARGET_COLUMNS)
